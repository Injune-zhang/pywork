...
//	var speciald=jQuery.parseJSON($("#dateList").val());
	...
	$(".jqeury_calendar_staticcheck").datetimepicker({
...
// $(document).on('click','#btn_staticcheck',function(){     //点击查看按钮
	// staticcheck_date=$("#staticcheck_date").val();
	// project_name=$("#projectCurName").val();
	// //检查分类默认设为第一个
	// errorid="1";
	// get_overview_staticdata(project_name,errorid,staticcheck_date);	
// });
...
$(document).on('click','.collapsed',function(){     //点击检查分页
	...
	var isMore=0;
	get_overview_morestaticdata(project_name,errorid,staticcheck_date,isMore);	
}); 
...
function get_overview_staticdata(project_name,errorid,staticcheck_date){   //加载新查看的内容
	$.get('get_overview_staticdata', {
        'staticcheck_date':staticcheck_date,
...
function get_overview_morestaticdata(project_name,errorid,staticcheck_date,isMore){  //加载更多错误
	$.get('get_overview_morestaticdata',{
        'staticcheck_date':staticcheck_date,
        'project_name':project_name,
        'errorid':errorid,	
        'isMore':isMore,
