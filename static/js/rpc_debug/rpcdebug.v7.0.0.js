...
function toEdit(){
    ...
    console.log(JSON.stringify(rpc_for_send['args']))
...
function changeToNewrpcs(){
    ismain=false
    $('#screen .left').stop()
    $('.toptab').removeClass('ontoptab')
    $('#tonewrpcs').addClass('ontoptab')
    $('._tab').hide()
    $('#_Newrpclist').load('/rpc_debug/getnewrpcs/')
    $('#Newrpclist').show()
}
...
function addRpcArgs(id,explain)
{
	$.ajax({
      type: 'POST',
      url: '/rpc_debug/saverpc_args/',
      data: {'rpc_explain':explain,'rpc_id':id},
      success: function(re){
        alert(re)
      },
    });
...
//确认新增rpc
function delRPCfromlist(rpcname,e){
    $.ajax({
      type: 'POST',
      url: '/rpc_debug/confirm_rpc/',
      data: {'name':rpcname,'add':true},
      success: function(re){
        showretip(re)
        if(re=='删除成功'){
