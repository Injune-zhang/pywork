define("echarts/chart/force", ["require", "../component/base", "./base", "../data/Graph", "../layout/Force", "zrender/shape/Line", "zrender/shape/BezierCurve", "zrender/shape/Image", "../util/shape/Icon", "../config", "../util/ecData", "zrender/tool/util", "zrender/config", "zrender/tool/vector", "../chart"], function(e) {
    "use strict";

    function t(e, t, o, d, c) {
        var p = this;
        s.call(this, e, t, o, d, c), r.call(this), this.__nodePositionMap = {}, this._graph = new l(!0), this._layout = new h, this._layout.onupdate = function() {
            p._step()
    ...
    var s = e("../component/base"),
        ...
        l = e("../data/Graph"),
        h = e("../layout/Force"),
        d = e("zrender/shape/Line"),
        c = e("zrender/shape/BezierCurve"),
        p = e("zrender/shape/Image"),
        m = e("../util/shape/Icon"),
        u = e("../config"),
        g = e("../util/ecData"),
    ...
    return t.prototype = {
        ...
        _initSerie: function(e, t) {
            this._temperature = 1, this._graph = e.data ? this._getSerieGraphFromDataMatrix(e) : this._getSerieGraphFromNodeLinks(e), this._buildLinkShapes(e, t), this._buildNodeShapes(e, t), this.zr.modLayer(this.getZlevelBase(), {
                panable: e.roam === !0 || "move" === e.roam,
                zoomable: e.roam === !0 || "scale" === e.roam
        ...
        _getSerieGraphFromDataMatrix: function(e) {
            ...
            for (var o = e.data || e.nodes, a = 0; a < o.length; a++) {
                var s = {},
                    r = o[a];
                for (var h in r) "name" === h ? s.id = r.name : s[h] = r[h];
                var d = this._getNodeCategory(e, r),
                    c = d ? d.name : r.name;
                if (this.selectedMap[c] = this.isSelected(c), this.selectedMap[c]) t.push(s), i++;
                else {
                    n.splice(i, 1);
                    for (var p = 0; p < n.length; p++) n[p].splice(i, 1)
                }
            }
            var m = l.fromMatrix(t, n, !0);
        ...
        _getSerieGraphFromNodeLinks: function(e) {
            for (var t = new l(!0), i = e.data || e.nodes, n = 0, a = i.length; a > n; n++) {
                var o = i[n];
                if (o && !o.ignore) {
                    var s = this._getNodeCategory(e, o),
                        r = s ? s.name : o.name;
                    if (this.selectedMap[r] = this.isSelected(r), this.selectedMap[r]) {
        ...
        _initLayout: function(e) {
            ...
            this._steps = e.steps || 1, this._layout.center = this.parseCenter(this.zr, e.center), this._layout.width = this.parsePercent(e.size, this.zr.getWidth()), this._layout.height = this.parsePercent(e.size, this.zr.getHeight()), this._layout.large = e.large, this._layout.scaling = e.scaling, this._layout.ratioScaling = e.ratioScaling, this._layout.gravity = e.gravity, this._layout.temperature = 1, this._layout.coolDown = e.coolDown, this._layout.preventNodeEdgeOverlap = e.preventOverlap, this._layout.preventNodeOverlap = e.preventOverlap;
            for (var s = 1 / 0, r = -1 / 0, l = 0; i > l; l++) {
                var h = t.nodes[l];
                r = Math.max(h.layout.size, r), s = Math.min(h.layout.size, s)
            }
            for (var d = r - s, l = 0; i > l; l++) {
                var h = t.nodes[l];
                d > 0 ? (h.layout.size = (h.layout.size - s) * (a - n) / d + n, h.layout.mass = h.layout.size / a) : (h.layout.size = (a - n) / 2, h.layout.mass = .5)
            }
            for (var l = 0; i > l; l++) {
                var h = t.nodes[l];
                if ("undefined" != typeof this.__nodePositionMap[h.id]) h.layout.position = U.create(), U.copy(h.layout.position, this.__nodePositionMap[h.id]);
                else if ("undefined" != typeof h.data.initial) h.layout.position = U.create(), U.copy(h.layout.position, h.data.initial);
                else {
                ...
                var m = h.shape.style,
                    u = h.layout.size;
                m.width = m.width || 2 * u, m.height = m.height || 2 * u, m.x = -m.width / 2, m.y = -m.height / 2, U.copy(h.shape.position, h.layout.position)
            }
            i = t.edges.length, r = -1 / 0;
            for (var l = 0; i > l; l++) {
                var g = t.edges[l];
                g.layout.weight > r && (r = g.layout.weight)
            }
            for (var l = 0; i > l; l++) {
                var g = t.edges[l];
                g.layout.weight /= r
        ...
        _buildNodeShapes: function(e, t) {
            ...
            i.eachNode(function(i) {
                var a = this._getNodeCategory(e, i.data),
                    o = [i.data, a, e],
                    s = this._getNodeQueryTarget(e, i.data),
                    r = this._getNodeQueryTarget(e, i.data, "emphasis"),
                    l = new m({
                        style: {
                            x: 0,
                            y: 0,
                        ...
                        highlightStyle: {
                            color: this.deepQuery(r, "color", "emphasis"),
                            strokeColor: this.deepQuery(r, "strokeColor", "emphasis") || this.deepQuery(r, "borderColor", "emphasis"),
                            lineWidth: this.deepQuery(r, "lineWidth", "emphasis") || this.deepQuery(r, "borderWidth", "emphasis")
                        },
                        clickable: e.clickable,
                        zlevel: this.getZlevelBase()
                    });
                l.style.color || (l.style.color = this.getColor(a ? a.name : i.id)), l.style.iconType = this.deepQuery(o, "symbol"), l.style.width = l.style.height = 2 * (this.deepQuery(o, "symbolSize") || 0), l.style.iconType.match("image") && (l.style.image = l.style.iconType.replace(new RegExp("^image:\\/\\/"), ""), l = new p({
                    style: l.style,
                ...
                g.pack(l, e, t, i.data, i.rawIndex, i.data.name || "", i.category), this.shapeList.push(l), this.zr.addShape(l), i.shape = l
            }, this)
        },
        _buildLinkShapes: function(e, t) {
            for (var i = this._graph, n = i.edges.length, a = 0; n > a; a++) {
                var o = i.edges[a],
                    ...
                    r = o.node1,
                    l = o.node2,
                    h = this._getEdgeQueryTarget(e, o),
                    p = this.deepQuery(h, "type");
                ...
                if (V.merge(y.style, this.query(e, "itemStyle.normal.linkStyle"), !0), V.merge(y.highlightStyle, this.query(e, "itemStyle.emphasis.linkStyle"), !0), "undefined" != typeof s.itemStyle && (s.itemStyle.normal && V.merge(y.style, s.itemStyle.normal, !0), s.itemStyle.emphasis && V.merge(y.highlightStyle, s.itemStyle.emphasis, !0)), y.style.lineWidth = y.style.lineWidth || y.style.width, y.style.strokeColor = y.style.strokeColor || y.style.color, y.highlightStyle.lineWidth = y.highlightStyle.lineWidth || y.highlightStyle.width, y.highlightStyle.strokeColor = y.highlightStyle.strokeColor || y.highlightStyle.color, g.pack(y, e, t, o.data, null == o.rawIndex ? a : o.rawIndex, o.data.name || r.id + " - " + l.id, r.id, l.id), this.shapeList.push(y), this.zr.addShape(y), o.shape = y, e.linkSymbol && "none" !== e.linkSymbol) {
                    var U = new m({
                        style: {
                            ...
                            opacity: y.style.opacity,
                            shadowBlur: y.style.shadowBlur,
                            shadowColor: y.style.shadowColor,
                            shadowOffsetX: y.style.shadowOffsetX,
                            shadowOffsetY: y.style.shadowOffsetY
                        },
                        highlightStyle: {
                            brushType: "fill"
                        },
                    ...
                    y._symbolShape = U, this.shapeList.push(U), this.zr.addShape(U)
                }
            }
        },
        _updateLinkShapes: function() {
            for (var e = U.create(), t = this._graph.edges, i = 0, n = t.length; n > i; i++) {
                var a = t[i],
                    o = a.node1.shape,
                    s = a.node2.shape,
                    r = o.position,
                    l = s.position;
                if (a.shape.style.xStart = r[0], a.shape.style.yStart = r[1], a.shape.style.xEnd = l[0], a.shape.style.yEnd = l[1], "bezier-curve" === a.shape.type && (a.shape.style.cpX1 = (r[0] + l[0]) / 2 - (l[1] - r[1]) / 4, a.shape.style.cpY1 = (r[1] + l[1]) / 2 - (r[0] - l[0]) / 4), a.shape.modSelf(), a.shape._symbolShape) {
                    var h = a.shape._symbolShape;
                    U.copy(h.position, s.position), U.sub(e, o.position, s.position), U.normalize(e, e), U.scaleAndAdd(h.position, h.position, e, s.style.width / 2 + 2);
                    var d = Math.atan2(e[1], e[0]);
                    h.rotation = Math.PI / 2 - d, h.modSelf()
                }
            }
        },
        _syncNodePositions: function() {
            for (var e = this._graph, t = 0; t < e.nodes.length; t++) {
                var i = e.nodes[t],
                    ...
                    a = i.data,
                    o = i.shape;
                o.fixed || a.fixX && a.fixY ? U.copy(n, o.position) : a.fixX ? (n[0] = o.position[0], o.position[1] = n[1]) : a.fixY ? (n[1] = o.position[1], o.position[0] = n[0]) : 0 == isNaN(a.fixX - 0) && 0 == isNaN(a.fixY - 0) ? (o.position[0] += (n[0] - o.position[0]) * a.fixX, n[0] = o.position[0], o.position[1] += (n[1] - o.position[1]) * a.fixY, n[1] = o.position[1]) : 0 == isNaN(a.fixX - 0) ? (o.position[0] += (n[0] - o.position[0]) * a.fixX, n[0] = o.position[0], o.position[1] = n[1]) : 0 == isNaN(a.fixY - 0) ? (o.position[1] += (n[1] - o.position[1]) * a.fixY, n[1] = o.position[1], o.position[0] = n[0]) : U.copy(o.position, n);
                var s = a.name;
                if (s) {
                    var r = this.__nodePositionMap[s];
                    r || (r = this.__nodePositionMap[s] = U.create()), U.copy(r, n)
                }
                o.modSelf()
            }
        },
        _step: function() {
            this._syncNodePositions(), this._updateLinkShapes(), this.zr.refreshNextFrame(), this._layout.temperature > .01 ? this._layout.step(this._steps) : this.messageCenter.dispatch(u.EVENT.FORCE_LAYOUT_END, {}, {}, this.myChart)
        },
        refresh: function(e) {
    ...
    }, V.inherits(t, r), V.inherits(t, s), e("../chart").define("force", t), t
                ...
                for (var l = s; a > l; l++) {
                    ...
                    if (0 !== h) {
                        var d = o.nodes[s],
                            c = o.nodes[l],
                            p = o.addEdge(d, c, {});
                        if (p.data.weight = h, s !== l && n && t[l][s]) {
                            var m = o.addEdge(c, d, {});
                            m.data.weight = t[l][s]
                        }
                    }
    ...
    var n, a = e("./forceLayoutWorker"),
        o = e("zrender/tool/vector"),
    ...
    return l.prototype.updateConfig = function() {
        var e = this.width,
            t = this.height,
            i = Math.min(e, t),
            n = {
        ...
        } else this._layout || (this._layout = new a), this._layoutWorker && (this._layoutWorker.terminate(), this._layoutWorker = null);
        this.temperature = 1, this.graph = e;
        for (var s = e.nodes.length, l = new r(2 * s), h = new r(s), d = new r(s), c = 0; s > c; c++) {
            var p = e.nodes[c];
            l[2 * c] = p.layout.position[0], l[2 * c + 1] = p.layout.position[1], h[c] = "undefined" == typeof p.layout.mass ? 1 : p.layout.mass, d[c] = "undefined" == typeof p.layout.size ? 1 : p.layout.size, p.layout.__index = c
        }
        s = e.edges.length;
        for (var m = new r(2 * s), u = new r(s), c = 0; s > c; c++) {
            var g = e.edges[c];
            m[2 * c] = g.node1.layout.__index, m[2 * c + 1] = g.node2.layout.__index, u[c] = g.layout.weight || 1
        }
        this._token = t(), this._layoutWorker ? this._layoutWorker.postMessage({
            cmd: "init",
            nodesPosition: l,
            nodesMass: h,
            nodesSize: d,
            ...
            edgesWeight: u,
            token: this._token
        }) : (this._layout.setToken(this._token), this._layout.initNodes(l, h, d), this._layout.initEdges(m, u)), this.updateConfig()
    }, l.prototype.step = function(e) {
        var t = this.graph.nodes;
        if (this._layoutWorker) {
            for (var i = new r(2 * t.length + 1), n = 0; n < t.length; n++) {
                var a = t[n];
                i[2 * n + 1] = a.layout.position[0], i[2 * n + 2] = a.layout.position[1]
            }
        ...
        if (this._layoutWorker) {
            ...
            if (i === this._token) {
                for (var n = 0; n < this.graph.nodes.length; n++) {
                    ...
                    a.layout.position[0] = t[2 * n + 1], a.layout.position[1] = t[2 * n + 2]
                }
            ...
            for (var n = 0; n < this.graph.nodes.length; n++) {
                var a = this.graph.nodes[n];
                o.copy(a.layout.position, this._layout.nodes[n].position)
            }
            this.onupdate && this.onupdate()
        }
    ...
    function n() {
        this.position = s.create(), this.force = s.create(), this.forcePrev = s.create(), this.speed = s.create(), this.speedPrev = s.create(), this.mass = 1, this.inDegree = 0, this.outDegree = 0
    }
    ...
    function o() {
        this.barnesHutOptimize = !1, this.barnesHutTheta = 1.5, this.repulsionByDegree = !1, this.preventNodeOverlap = !1, this.preventNodeEdgeOverlap = !1, this.strongGravity = !0, this.gravity = 1, this.scaling = 1, this.edgeWeightInfluence = 1, this.center = [0, 0], this.width = 500, this.height = 500, this.maxSpeedIncrease = 1, this.nodes = [], this.edges = [], this.bbox = new l(4), this._rootRegion = new i, this._rootRegion.centerOfMass = s.create(), this._massArr = null, this._k = 0
    }
    var s, r = "undefined" == typeof window && "undefined" == typeof t;
    s = r ? {
        create: function(e, t) {
    ...
    if (i.prototype.beforeUpdate = function() {
            ...
            if (!t) {
                var n = (i[0] + i[2]) / 2,
                    ...
                    o = (i[2] - i[0]) / 2,
                    s = (i[3] - i[1]) / 2,
                    r = e.position[0] >= n ? 1 : 0,
                    l = e.position[1] >= a ? 1 : 0,
                    t = this._newSubRegion();
                t.setBBox(r * o + i[0], l * s + i[1], (r + 1) * o + i[0], (l + 1) * s + i[1])
            }
            t.addNode(e)
            ...
            for (var o = "undefined" != typeof i, s = 0; a > s; s++) {
                var r = new n;
                r.position[0] = e[2 * s], r.position[1] = e[2 * s + 1], r.mass = t[s], o && (r.size = i[s]), this.nodes.push(r)
            }
            ...
            if (this.updateBBox(), this._k = .4 * this.scaling * Math.sqrt(this.width * this.height / e), this.barnesHutOptimize) {
                ...
                for (var t = 0; e > t; t++) {
                    var a = this.nodes[t];
                    i += a.mass, s.scaleAndAdd(n, n, a.position, a.mass)
                }
                i > 0 && s.scale(n, n, 1 / i)
            }
            ...
            for (var e = this.nodes.length, t = 0; e > t; t++) {
                var i = this.nodes[t];
                s.copy(i.forcePrev, i.force), s.copy(i.speedPrev, i.speed), s.set(i.force, 0, 0)
            }
            ...
            for (var e = this.nodes.length, t = s.create(), i = 0; e > i; i++) {
                var n = this.nodes[i],
                    a = n.speed;
                s.scale(n.force, n.force, 1 / 30);
                var o = s.len(n.force) + .1,
                    r = Math.min(o, 500) / o;
                s.scale(n.force, n.force, r), s.add(a, a, n.force), s.scale(a, a, this.temperature), s.sub(t, a, n.speedPrev);
                var l = s.len(t);
                if (l > 0) {
                    s.scale(t, t, 1 / l);
                    var h = s.len(n.speedPrev);
                    h > 0 && (l = Math.min(l / h, this.maxSpeedIncrease) * h, s.scaleAndAdd(a, n.speedPrev, t, l))
                }
                var d = s.len(a),
                    r = Math.min(d, 100) / (d + .1);
                s.scale(a, a, r), s.add(n.position, n.position, a)
            }
            ...
            return function(t, i, n) {
                if (t !== i && (0 !== t.mass || 0 !== i.mass)) {
                    s.sub(e, t.position, i.position);
                    var a = e[0] * e[0] + e[1] * e[1];
                    if (0 !== a) {
                        var o, r = t.mass + i.mass,
                            l = Math.sqrt(a);
                        s.scale(e, e, 1 / l), this.preventNodeOverlap ? (l = l - t.size - i.size, l > 0 ? o = this.nodeToNodeRepulsionFactor(r, l, this._k) : 0 >= l && (o = this._k * this._k * 10 * r)) : o = this.nodeToNodeRepulsionFactor(r, l, this._k), n || s.scaleAndAdd(t.force, t.force, e, 2 * o), s.scaleAndAdd(i.force, i.force, e, 2 * -o)
                    }
                }
            }
            ...
            return function(t) {
                ...
                if (!(this.preventOverlap && (o = o - i.size - n.size, 0 >= o))) {
                    ...
                    s.scaleAndAdd(i.force, i.force, e, -r), s.scaleAndAdd(n.force, n.force, e, r)
                }
            }
        }(), o.prototype.applyNodeGravity = function() {
            var e = s.create();
            return function(t) {
                s.sub(e, this.center, t.position), this.width > this.height ? e[1] *= this.width / this.height : e[0] *= this.height / this.width;
                var i = s.len(e) / 100;
                this.strongGravity ? s.scaleAndAdd(t.force, t.force, e, i * this.gravity * t.mass) : s.scaleAndAdd(t.force, t.force, e, this.gravity * t.mass / (i + 1))
            }
            ...
            return function(n, a) {
                var o = n.node1,
                    r = n.node2;
                if (o !== a && r !== a) {
                    s.sub(e, r.position, o.position), s.sub(t, a.position, o.position);
                    var l = s.len(e);
                    s.scale(e, e, 1 / l);
                    var h = s.dot(e, t);
                    if (!(0 > h || h > l)) {
                        s.scaleAndAdd(i, o.position, e, h);
                        var d = s.dist(i, a.position) - a.size,
                            c = this.edgeToNodeRepulsionFactor(a.mass, Math.max(d, .1), 100);
                        s.sub(e, a.position, i), s.normalize(e, e), s.scaleAndAdd(a.force, a.force, e, c), s.scaleAndAdd(o.force, o.force, e, -c), s.scaleAndAdd(r.force, r.force, e, -c)
                    }
                }
            }
        ...
        }, o.prototype.setToken = function(e) {
            this._token = e
        }, o.prototype.tokenMatch = function(e) {
        ...
        self.onmessage = function(e) {
            if (e.data instanceof ArrayBuffer) {
                if (!h) return;
                for (var t = new Float32Array(e.data), i = (t.length - 1) / 2, n = 0; i > n; n++) {
                    var a = h.nodes[n];
                    a.position[0] = t[2 * n + 1], a.position[1] = t[2 * n + 2]
                }
            } else switch (e.data.cmd) {
                case "init":
                    h || (h = new o), h.initNodes(e.data.nodesPosition, e.data.nodesMass, e.data.nodesSize), h.initEdges(e.data.edges, e.data.edgesWeight), h._token = e.data.token;
                    break;
                case "updateConfig":
                    if (h)
                        for (var s in e.data.config) h[s] = e.data.config[s];
                    break;
                case "update":
                    var r = e.data.steps;
                    if (h) {
                        var i = h.nodes.length,
                            t = new Float32Array(2 * i + 1);
                        h.temperature = e.data.temperature;
                        for (var n = 0; r > n; n++) h.update(), h.temperature *= e.data.coolDown;
                        for (var n = 0; i > n; n++) {
                            var a = h.nodes[n];
                            t[2 * n + 1] = a.position[0], t[2 * n + 2] = a.position[1]
                        }
                        t[0] = h._token, self.postMessage(t.buffer, [t.buffer])
                    } else {
